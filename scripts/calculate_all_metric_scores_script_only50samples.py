...
def calculate_metrics_for_all_columns_in_file(summary_file, score_file, column_to_leave=None):
    ...
    cols = ["model", "dataset", "domain", "rouge1", "rouge2", "rougeL", "bertscore_f1_mean", "bleu", "meteor"]
    rouge = rouge_metric()
    ...
    #         if score_file.endswith(".xlsx"):
    #             m_df.to_excel(score_file.format(_metric), index=False)
    #         else:
    ...
    if not os.path.exists(score_file):
        ...
        if score_file.endswith(".xlsx"):
            m_df.to_excel(score_file, index=False)
        else:
            m_df.to_csv(score_file, index=False)
    ...
    for _peft in pefts:
        ...
        model_name = _peft if "multiple_" in _peft else summary_file.split("summaries_")[1].split("_50samples")[0] + "_" + _peft
        ...
        ds, dom = summary_file.split("summaries_")[1].split("_50samples")[0].split("_")
        metric_scores_final = {
            "model": model_name,
            "dataset": ds,
            "domain": dom,
            ...
            "bleu": bl_score["bleu"],
            "meteor": m_score["meteor"],
            "bertscore_f1_mean": bs_score["f1"]["mean"]
        }

        print("Scores for file: {} and peft {}\n".format(summary_file, _peft))

        m_df = pd.read_csv(score_file) if score_file.endswith(".csv") else pd.read_excel(score_file)

        m_df = pd.concat([m_df, pd.DataFrame([metric_scores_final])], ignore_index=True)

        m_df.to_csv(score_file, index=False) if score_file.endswith(".csv") else m_df.to_excel(score_file, index=False)

    print("***** Scores for file: {} are written to {} ******".format(summary_file, score_file))
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        score_file = "summaries/50scores/combined_scores50.xlsx"
        calculate_metrics_for_all_columns_in_file(summary_file=file, score_file=score_file,
