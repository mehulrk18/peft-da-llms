    ...
    for domain, datasets in datasets_info_dict.items():
        for _dataset in datasets.keys():
            ...
            if domain.name.lower() == "unseen_test":
                ...
                test_df["content_len"] = [len(x) for x in data["content"].tolist()]
                test_df["summary_len"] = [len(x) for x in data["summary"].tolist()]
            else:
                data = SumDataLoader(
                ...
                train_df["content_len"] = [len(x) for x in data.train_set["content"]]
                train_df["summary_len"] = [len(x) for x in data.train_set["summary"]]
                ...
                val_df["content_len"] = [len(x) for x in data.validation_set["content"]]
                val_df["summary_len"] = [len(x) for x in data.validation_set["summary"]]
                ...
                test_df["content_len"] = [len(x) for x in data.test_set["content"]]
                test_df["summary_len"] = [len(x) for x in data.test_set["summary"]]
            ...
            obj = {
                ...
                "train_average_content_len": train_df["content_len"].mean() if len(train_df) > 0 else 0,
                "train_average_summary_len": train_df["summary_len"].mean() if len(train_df) > 0 else 0,
                ...
                "validation_average_content_len": val_df["content_len"].mean() if len(val_df) > 0 else 0,
                "validation_average_summary_len": val_df["summary_len"].mean() if len(val_df) > 0 else 0,
                ...
                "test_average_content_len": test_df["content_len"].mean() if len(test_df) > 0 else 0,
                "test_average_summary_len": test_df["summary_len"].mean() if len(test_df) > 0 else 0
            }
...
if __name__ == "__main__":
    ...
    calculate_stats(llama, used_samples, "results/dataset_used_samples_stats_len.xlsx")
    calculate_stats(llama, max_samples, "results/dataset_max_samples_stats_len.xlsx")
