...
def testing_model(llama_model, llama_tokenizer, domain, peft_name):
    ...
    try:
        with open("random_text_{}.txt".format(peft_name), "w") as f:
    ...
    file_name = "Test_summaries_{}.csv".format(peft_name)
...
if __name__ == "__main__":
    ...
    peft_name = "_".join(trained_peft_path.split("_")[:2])
    llama_model.load_adapter(trained_peft_path)
    ...
    testing_model(llama_model=llama_model, llama_tokenizer=llama_tokenizer, domain=domain, peft_name=trained_peft_path.split("/")[-1])
